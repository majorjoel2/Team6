...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (fireTest_M->Timing.clockTick0+
    fireTest_M->Timing.clockTickH0* 4294967296.0);
  fireTest_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(fireTest_M, 0);
  ...
  errorCode = extmodeInit(fireTest_M->extModeInfo, &rtmGetTFinal(fireTest_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(fireTest_M, true);
    }
  }
